...
from rust_companion_plus.ui.notification_settings_panel import (
    MarketplaceNotificationSettingsPanel,
)
...
class DashboardTab(ctk.CTkScrollableFrame):
    def __init__(self, master, context):
        ...
        ctk.CTkButton(
            ...
            border_width=1,
            command=self.open_feature_guide,
        ).grid(row=0, column=3, sticky="e", pady=(0, 12))
        ...
        notifications = SectionCard(
            ...
            (
                "Choose progression presets, watched items, deal thresholds, "
                "stock/cost limits, and fullscreen or in-app delivery."
            ),
        )
        notifications.grid(
            row=5,
        ...
        self.notification_panel = MarketplaceNotificationSettingsPanel(
            notifications,
            context,
        )
        self.notification_panel.grid(
    ...
    def open_feature_guide(self) -> None:
        app = getattr(self.context, "app", None)
        opener = getattr(app, "open_guide", None)
        if callable(opener):
            opener("Overview")

    def _notification_settings_from_controls(self) -> dict:
        return self.notification_panel.values()
